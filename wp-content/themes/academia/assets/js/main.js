...
(function ($) {
    ...
    G5Plus.onReady = {
        init: function () {
            ...
            jQuery('.barion-recurrent.single').click(function (e) {
                e.preventDefault();
                var firstname = jQuery('input[name="billing_first_name"]').val();

                var lastname = jQuery('input[name="billing_last_name"]').val();
                var email =jQuery('input[name="billing_email"]').val();
                var price = jQuery('.order-total .woocommerce-Price-amount').text();
                price = price.slice(0,-2);
                window.location = '/bejelentkezes-regisztracio?fn=' + firstname + "&ln="+lastname+"&email="+email+"&price="+price+"&reg";
            });


            jQuery('#szemelyes_adatok').collapsable();
            jQuery('#jelszo_valtoztatas').collapsable();

            var url = new URL(window.location.href);

            if (url.searchParams.get('price')) {
                 var fn = url.searchParams.get('fn');
                 jQuery("#reg_billing_first_name").val(fn);
                 var ln = url.searchParams.get('ln');
                jQuery("#reg_billing_last_name").val(ln);

                 var email = url.searchParams.get('email');
                jQuery("#reg_email").val(email);

                 var price = url.searchParams.get('price');
                price = price.replace(/,/g, "");
                jQuery("#acf-field_5cb5d089e9dfc").val(parseInt(price));

            }
            jQuery(".barion-recurrent.first").click(function(e){
                if(!jQuery("#recurrent-checkbox").is(':checked')){
                    e.preventDefault();
                    jQuery("#checkbox-box").addClass("red")
                }
            })
        }
    };
